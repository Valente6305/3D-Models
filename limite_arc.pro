pro limite_arc,st_sun,limite,n_phase=n_phase, n_linhas=n_linhas,debug=debug
...
;  T3D, /reset, translate=[0.,0.,0.0],Rotate=[0.,zz[i]+st_sun.a_inc,0.],matrix=tmp 
;  T3D, tmp,Translate=[0.,0.,-st_sun.deep*st_sun.r_sun],matrix=tmp
;  T3D, tmp, Rotate=[st_sun.d_ang,st_sun.a_inc,90.-st_sun.azim],matrix=tmp
;  T3D, tmp, Translate=[0.,0.,st_sun.l_arc*st_sun.r_sun],matrix=tmp 
;  T3D, tmp, Rotate=[-st_sun.B_lat,0.,0.],matrix=tmp 
...
;    if j eq 0 then begin
 ;      meio_x=(max(xx[j,*],min=mmx)+mmx)/2.0
  ;     meio_y=(max(yy[j,*],min=mmy)+mmy)/2.0
   ;    meio_z=(max(zz[j,*],min=mmz)+mmz)/2.0
    ;   elonx=max(abs(xx[j,*]-meio_x))
     ;  elony=max(abs(yy[j,*]-meio_y))
      ; elonz=max(abs(zz[j,*]-meio_z))
      ; elong=max([elonx,elony,elonz])*1.05/st_sun.r_sun
      ; meio_center=[[meio_x],[meio_y],[meio_z]]
   ; endif
...
if keyword_set(debug) then begin
    ...
    plots,x_c[j],y_c[j],psy=2,col=150
...
; ======================= FIM do DEBUG ===========================
 ...
 for i=0,(n_phase-1)*0.0 do x_min[i]=max([abs(xx[i,*]-x_c[i]),$
...
; limite={x_min:elong, center:meio_center,coord:cop,x0:x0,y0:y0,z0:z0,$ 
;         r0:m_r0,lamb:m_lamb,spot:cc,flag_error:flag_error, $
;         dip_coord:dip_coord[0:2]}
end
