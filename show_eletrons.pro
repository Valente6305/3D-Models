...
pro show_eletrons,st_arc,px, graf=graf, nome=nome
...
; parte do grayplot
if keyword_set(nome) then begin
   if n_elements(nome) eq 0 then name='Electrons'
   n=n_elements(imagem.image)
   sz=size(imagem.image[*].imagem[*,*,*])
   nel=reform(mag_res.nel)
   pp=where(nel gt 0)
   if pp[0] ne -1 then nel[pp]=alog10(nel[pp])
;stop
   nel=total(nel,3)
   stokes_I=fltarr(sz[2],sz[3])
   stokes_I=nel
   spix=128
   nsq=1
...
grayplot,reform(stokes_I[*,*]),xarr=(arr+x0)*960., yarr=(arr+y0)*960.0 ; arcsec
...
;================================fim do grayplot==============================
      ...
      plots,cc2[0,0],cc2[0,1],psy=3,col=255
;stop
erro:
      if keyword_set(graf) then begin
         set_isoxy,0,0,0,0
